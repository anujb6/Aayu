 ...
 * Stage 1 = 0 (new users can evolve same day if XP met)
 * Higher stages require more time to prevent speed-running
 */
export const MIN_DAYS_PER_STAGE = {
  1: 0,   // Baby stage - can evolve immediately with enough XP
  2: 2,   // Need 2+ days in stage
  3: 4,   // Need 4+ days in stage
  4: 6,   // Need 6+ days in stage
  5: 8,   // Need 8+ days in stage
  6: 0    // Max level
}
...
export function evolve(creature) {
  ...
  // Defensive bounds check - max stage is 6
  if (creature.stage >= 6) {
    return {
      evolved: false,
      oldStage: creature.stage,
  ...
  // Evolve - carry over excess XP to new stage (with bounds check)
  creature.stage = Math.min(6, creature.stage + 1)
